    ...
    >
      <el-table-column label="Name" min-width="180" show-overflow-tooltip>
        <template slot-scope="scope">
          <router-link :to="'/workload/job-edit?namespace=' + scope.row.namespace + '&name=' + scope.row.name" class="link-type">
      ...
      <el-table-column label="Completions" min-width="120" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.completions }}</span>
        </template>
      </el-table-column>
      <el-table-column label="Succeeded" min-width="120" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.succeeded }}</span>
        </template>
      </el-table-column>
...
export default {
  ...
  watch: {
    ...
    '$route'(to, from) {
      ...
      if (toPath === '/workload/jobs') {
        // If returning from edit/create page, check if there's a namespace in the query
        if (from && (fromPath.includes('job-edit') || fromPath.includes('job-create'))) {
  ...
  methods: {
    ...
    async getList() {
      ...
      this.listLoading = true
      try {
        await this.$store.dispatch('job/getJobList', this.listQuery)
        // List will be updated via the watcher
      } catch (error) {
        console.error('Failed to fetch job list:', error)
        this.$message.error('Failed to fetch job list')
      } finally {
        this.listLoading = false
      }
    },
    ...
    handleCreate() {
      this.$router.push({ 
        path: '/workload/job-create',
        query: { namespace: this.listQuery.namespace } 
      })
    },
    handleUpdate(row) {
      this.$router.push({ 
        path: '/workload/job-edit', 
        query: { namespace: row.namespace, name: row.name }
      })
    },
    handleDelete(row) {
      this.$confirm('Confirm to delete the job?', 'Warning', {
