  ...
  <div class="app-container">
    <el-form ref="form" :model="form" label-width="120px">
      ...
      <el-alert type="info" show-icon>
        Pod template configuration defines the containers to run in this daemonset
      </el-alert>
      ...
      <el-tabs v-model="activeTab" type="card">
        ...
        <el-tab-pane label="Containers" name="containers">
          <el-alert type="info" show-icon>
            Define the main containers for this daemonset
          </el-alert>
...
export default {
  ...
  methods: {
    ...
    async getDaemonSetDetail() {
      this.loading = true
      try {
        const response = await this.$store.dispatch('daemonset/getDaemonSetDetail', this.$route.query)
        if (response && response.data) {
          ...
          this.$message.error('Failed to get daemonset details: Empty response')
        }
      } catch (error) {
        console.error('Failed to get daemonset details:', error)
        this.$message.error('Failed to get daemonset details')
      } finally {
        this.loading = false
      }
    },
    ...
    onSubmit() {
      if (!this.form.name) {
        this.$message.error('Please enter a daemonset name')
        return
      }
      ...
      this.loading = true
      this.$store.dispatch('daemonset/createOrUpdateDaemonSet', this.form)
        .then(() => {
          this.$notify({
            title: 'Success',
            message: this.isEdit ? 'DaemonSet updated successfully' : 'DaemonSet created successfully',
            type: 'success',
            duration: 2000
          })
          this.$router.push({
