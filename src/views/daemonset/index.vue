  ...
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-input
        ...
        placeholder="Search daemonset name"
    ...
    >
      <el-table-column label="Name" min-width="180" show-overflow-tooltip>
        <template slot-scope="scope">
          <router-link :to="'/workload/daemonset-edit?namespace=' + scope.row.namespace + '&name=' + scope.row.name" class="link-type">
      ...
      <el-table-column label="Desired" min-width="100" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.desired }}</span>
        </template>
      </el-table-column>
      <el-table-column label="Current" min-width="80" align="center">
        <template slot-scope="scope">
...
export default {
  ...
  computed: {
    ...mapState({
      daemonsets: state => state.daemonset.daemonsets || []
    })
  },
  created() {
    this.initData()
  ...
  watch: {
    ...
    '$route'(to, from) {
      ...
      if (toPath === '/workload/daemonsets') {
        // If returning from edit/create page, check if there's a namespace in the query
        if (from && (fromPath.includes('daemonset-edit') || fromPath.includes('daemonset-create'))) {
  ...
  methods: {
    ...
    async getList() {
      ...
      this.listLoading = true
      try {
        await this.$store.dispatch('daemonset/getDaemonSetList', this.listQuery)
        // List will be updated via the watcher
      } catch (error) {
        console.error('Failed to fetch daemonset list:', error)
        this.$message.error('Failed to fetch daemonset list')
      } finally {
        this.listLoading = false
      }
    },
    ...
    handleCreate() {
      this.$router.push({ 
        path: '/workload/daemonset-create',
        query: { namespace: this.listQuery.namespace } 
      })
    },
    handleUpdate(row) {
      this.$router.push({ 
        path: '/workload/daemonset-edit', 
        query: { namespace: row.namespace, name: row.name }
      })
    },
    handleDelete(row) {
      this.$confirm('Confirm to delete the daemonset?', 'Warning', {
