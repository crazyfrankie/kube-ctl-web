  ...
  <div class="app-container">
    <el-form ref="form" :model="form" label-width="120px">
      ...
      <el-alert type="info" show-icon>
        Pod template configuration defines the containers to run in this deployment
      </el-alert>
      ...
      <el-tabs v-model="activeTab" type="card">
        ...
        <el-tab-pane label="Containers" name="containers">
          <el-alert type="info" show-icon>
            Define the main containers for this deployment
          </el-alert>
...
export default {
  ...
  methods: {
    ...
    async getDeploymentDetail() {
      this.loading = true
      try {
        const response = await this.$store.dispatch('deployment/getDeploymentDetail', this.$route.query)
        if (response && response.data) {
          ...
          this.$message.error('Failed to get deployment details: Empty response')
        }
      } catch (error) {
        console.error('Failed to get deployment details:', error)
        this.$message.error('Failed to get deployment details')
      } finally {
        this.loading = false
      }
    },
    ...
    onSubmit() {
      if (!this.form.name) {
        this.$message.error('Please enter a deployment name')
        return
      }
      ...
      this.loading = true
      this.$store.dispatch('deployment/createOrUpdateDeployment', this.form)
        .then(() => {
          this.$notify({
            title: 'Success',
            message: this.isEdit ? 'Deployment updated successfully' : 'Deployment created successfully',
            type: 'success',
            duration: 2000
          })
          this.$router.push('/workload/deployments')
          this.loading = false
        })
        .catch(error => {
          this.$notify({
