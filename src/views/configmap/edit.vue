<template>
  <div class="app-container">
    <div class="detail-header">
      <el-page-header @back="goBack" :content="isEdit ? '编辑ConfigMap' : '创建ConfigMap'" />
    </div>
    ...
    <el-card class="box-card" v-loading="loading">
      <el-form ref="form" :model="form" :rules="rules" label-width="120px">
        ...
        <el-form-item label="名称" prop="name" v-if="!isEdit">
          <el-input v-model="form.name" placeholder="请输入ConfigMap名称"></el-input>
        </el-form-item>
  ...
  methods: {
    ...
    async fetchData() {
      ...
      try {
        ...
        this.form.data = Object.entries(detail.data || {}).map(([key, value]) => ({ key, value }))
      } catch (error) {
    ...
    async onSubmit() {
      ...
      try {
        if (this.isEdit) {
          ...
          await this.$store.dispatch('configmap/createConfigMap', {
            namespace: this.form.namespace,
            name: this.form.name,
            data
          })
